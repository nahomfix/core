...
import { ArclightVideoLabel } from '../../__generated__/graphql'
...
describe('ArclightService', () => {
  let service: ArclightVideoService,
  ...
  beforeEach(async () => {
    ...
    service = module.get<ArclightVideoService>(ArclightVideoService)
    collectionMock = mockDeep()
    service.collection = collectionMock
  })

  describe('arclightVideoFilter', () => {
    it('should search with title', () => {
      const filter = {
        title: 'abc'
      }
      const response = service.arclightVideoFilter(filter)
    ...
    it('should filter by label', () => {
      const filter = {
        labels: [ArclightVideoLabel.collection]
      }
      const response = service.arclightVideoFilter(filter)
  ...
  it('should filter with subtitleLanguageIds', async () => {
    ...
    const response = await service.arclightVideoFilter(filter)
    expect(response.query).toContain(
  ...
  describe('getArclightVideo', () => {
    ...
    it('should return a video even without a langaugeId', async () => {
      expect(await service.getArclightVideo('20615')).toEqual(video)
      // should cache
      expect(await service.getArclightVideo('20615')).toEqual(video)
      expect(db.query).toHaveBeenCalledTimes(1)
    })
  ...
  describe('getArclightVideoBySlug', () => {
    it('should query the video by slug', async () => {
      ...
      expect(await service.getArclightVideoBySlug('jesus/english')).toEqual([])
      // should cache
      expect(await service.getArclightVideoBySlug('jesus/english')).toEqual([])
      expect(db.query).toHaveBeenCalledTimes(1)
    })
  ...
  describe('children', () => {
    ...
    it('should query', async () => {
      expect(await service.getArclightVideosByIds(['20615', '20616'])).toEqual([
        video,
        video
      ])
